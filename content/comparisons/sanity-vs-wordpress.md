...
* **Pick Sanity** as a net-new headless CMS for agent use cases. Wordpress's absence of outgoing webhooks in core, GROQ-style query expressiveness, and content-scoped credentials means rebuilding each of those layers outside the platform. That work belongs in the CMS, not the application.
